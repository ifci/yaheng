        ...
        if (IS_POST) {
            ...
            if ($act == "add") { //添加分类
                ...
                if ($M->where($data)->count() == 0) {
                    echo json_encode(($M->add($data)) ? array('status' => 1, 'info' => '添加成功', 'url' => U('Product/season', array('time' => time()))) : array('status' => 0, 'info' => '添加失败'));
                } else {
                    echo json_encode(array('status' => 0, 'info' => '名称已存在，请修改'));
                }
                ...
                echo json_encode(($M->save($data)) ? array('status' => 1, 'info' => '更新成功', 'url' => U('Product/season', array('time' => time()))) : array('status' => 0, 'info' => '更新失败'));
            } else if ($act == "del") { //删除分类
                echo json_encode(($M->where('cid='.$data['cid'])->delete()) ? array('status' => 1, 'info' => '删除成功', 'url' => U('Product/season', array('time' => time()))) : array('status' => 0, 'info' => '删除失败'.$data['pid']));
            }
        }else{
            $map['type'] = 's';
